...
class RmLeftArmOperator(Operator):
    def __init__(
        ...
        home_pose=np.array(self.robot.get_cartesian_position_euler())
    ...
    def _get_scaled_cart_pose(self, moving_robot_homo_mat):
        ...
        home_pose = self.robot.get_cartesian_position_euler()
    ...
    def _apply_retargeted_angles(self, log=False):
        ...
        final_pose[0:3]=final_pose[0:3]

        # Apply the filter
        if self.use_filter:
            final_pose = self.comp_filter(final_pose)
            
        print('calculate:',time.time()-time_start)
        ...
        # print(final_pose)
        # time = time.time()
        # print('Begin to apply the retargeted angles')
        # print('Arm_err:', self.robot.get_arm_err())
        # if self.robot.get_arm_err() == 4099:
        #     self.robot.clear_arm_err()
        # self.robot.control_arm_movejp(final_pose, threshold=0.25)

        # self.robot.move_coords(final_pose)# self.robot.arm_control(final_pose)
